...
let class_data = JSON.parse($('#class_data').val());
...
function booked_teacher() {
	...
	xhr.open("POST", "/staff/slot_appointment", true);
...
function get_subjects() {
	...
	xhr.open("GET", "/staff/getSubject", true);
	xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	xhr.onreadystatechange = function () {
	if (xhr.readyState == 4 && xhr.status == 200) {
...
// alert("pending");	 // 1234
  ...
  if (!course || !dept ) {
	...
	btn_dis(1);
	if(!cs_id) {  alert(cs_id); cs_id = $('#course_department option:selected').attr('id'); alert(cs_id); }
	var xhr;
	if (window.XMLHttpRequest) {
	xhr = new XMLHttpRequest();
	} else {
  ...
  $("#afternoon_break").val(after);$("#afternoon_break").attr('disabled','disabled');
  var lunch = table_datas[leng]; leng--;
  $("#lunch_break").val(lunch);$("#lunch_break").attr('disabled','disabled');
  var morning = table_datas[leng]; leng--;
  $("#morning_break").val(morning);$("#morning_break").attr('disabled','disabled');
  
  $("#break"+after+"").val(table_datas[leng--]); $("#break"+after+"").attr('disabled','disabled');
  $("#break"+lunch+"").val(table_datas[leng--]); $("#break"+lunch+"").attr('disabled','disabled');
  $("#break"+morning+"").val(table_datas[leng--]); $("#break"+morning+"").attr('disabled','disabled');
  ...
  $("input[name='th"+(j+1)+"']").attr('disabled','disabled');  
  }
...
function select_teacher(node){ return false;
	let available_teachers = not_booked_teachers(node);
