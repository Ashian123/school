...
function websdkready() {
  ...
  var join_meeting = document.getElementById("join_meeting");
  window.onload = function () {
    fetch("/getkeys")
      .then((res) => res.json())
      .then((res) => {
        localStorage.setItem("keys", res);
        API_KEY = res.apiKey;
        API_SECRET = res.apiSecret;
        join_meeting.click();
      });
  };
  join_meeting.addEventListener("click", (e) => {
    e.preventDefault();

    let url = window.location.href;
    const params = new URLSearchParams(url);

    document.getElementById("display_name").value = params.get("name");
    document.getElementById("meeting_number").value = params.get("mn");
    document.getElementById("meeting_pwd").value = params.get("pwd");
    document.getElementById("meeting_role").value = params.get("role");
    ...
    const signature = ZoomMtg.generateSignature({
      ...
      success: function (res) {
        ...
        const joinUrl = "/joinmeeting/?" + testTool.serialize(meetingConfig);
        console.log(joinUrl);
